    ...
    with transaction.atomic():
        try:  
            if request.method == "POST":
                ...
                new_billing_gstin = request.POST.get("new_billing_gstin","").strip()
                ...
                if billing_gstin_no == "" or new_billing_gstin != "":
                    billing_gstin_no = new_billing_gstin.strip()
                ...
                try:
                    ...
                    billing_gstin_no_obj, _ = PartyBillingGSTIN.objects.get_or_create(
                        party=party_details,
                        billing_gstin=billing_gstin_no,
                        )
                    proforma = ProformaInvoice.objects.create(
                        ...
                        billing_gstin_no=billing_gstin_no_obj,  
                        terms_note=terms,
...
def ProformaInvoicePageAJAX(request):
    ...
    try:
        ...
        if party_name:
            ...
            billing_gstin = list(Party.objects.filter(
                party_name__iexact=party_name,
            ).values_list("party_billing_gstin__billing_gstin").distinct())
