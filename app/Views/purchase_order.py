...
def view_purchase_order(request):
    ...
    with transaction.atomic():
        ...
        try:
                        ...
                        for job in jobs:
                            ws.append(
                                [
                                    ...
                                    obj.delivery_date,
                                    (
                                        obj.party_details.party_name
                                        if obj.party_details
                                        else ""
                                    ),
                                    obj.party_email.email if obj.party_email else "",
                                    (
                                        obj.party_contact.party_number
                                        if obj.party_contact
                                        else ""
                                    ),
                                    job.job_name,
                                    job.pouch_open_size,
            ...
            messages.warning(request, "something went wrong try again", e)
            logger.error(e)
        ...
        try:
            if request.method == "POST":
                if "delete_purchase_order" in request.POST:
                    ...
                    edit_purchase_order = get_object_or_404(
                        PurchaseOrder, id=purchase_order_id
                    )
                    edit_purchase_order.pouch_purchase_number = request.POST.get(
                        "pouch_purchase_number"
                    )
                    edit_purchase_order.delivery_date = request.POST.get(
                        "delivery_date"
                    )
                    edit_purchase_order.quantity_variate = request.POST.get(
                        "quantity_variate"
                    )
                    ...
                    pouch_combinations = [
                        s.strip() for s in request.POST.getlist("pouch_combination")
                    ]
                    ...
                    if "print_purchase_order" in request.POST:
                        ...
                        return render(
                            request, "Includes/purchase_order/print.html", context
                        )
        except Exception as e:
            messages.warning(request, "Something went wrong try again", e)
            logger.error(e)
...
def purchase_order_ajax(request):
    ...
    try:
        if request.method == "GET":
            ...
            total_ppb = 0
            jobs = list(
                PurchaseOrderJob.objects.filter(
                    purchase_order__party_details__party_name=party_name
                )
