        ...
        try:
            with transaction.atomic():
                if "save_quotation" in request.POST:
                    ...
                    pouch_combination = [
                        pc.strip() for pc in request.POST.getlist("pouch_combination[]")
                    ]
                    polyester_unit = request.POST.getlist("purchase_rate_unit[]")
                    quantities = request.POST.getlist("quantity[]")
                    purchase_rate_per_kg = request.POST.getlist(
                    ...
                    special_instructions = [
                        s.strip() for s in request.POST.getlist("special_instruction[]")
                    ]
                    delivery_addresses = [
                        s.strip() for s in request.POST.getlist("delivery_address[]")
                    ]
                    ...
                    if email_error:
                        messages.error(
                            request, email_error, extra_tags="custom-danger-style"
                        )
                        return redirect("quotation_page")
                    ...
                    # Party Block
                    party_details, party_email_obj, party_contact_obj = (
                        utils.get_or_create_party(
                            party_name, party_email, party_contact
                        )
                    )
    ...
    with transaction.atomic():
        try:
            if request.method == "GET":
                ...
                if party_id:
                    job_names = (
                        PouchQuotationJob.objects.filter(
                            quotation__party_details=party_id
                        )
                        .values("job_name")
                        .distinct()
                    )
                ...
                if job_id:
                    quotations = quotations.filter(
                        pouch_quotation_jobs__job_name=job_id
                    )
            ...
            if request.method == "POST":
                if "delete_quotation" in request.POST:
                    ...
                    edit_quotation.pouch_quotation_number = request.POST.get(
                        "pouch_quotation_number"
                    )
                    edit_quotation.delivery_date = request.POST.get("delivery_date")
                    edit_quotation.quantity_variate = request.POST.get(
                        "quantity_variate"
                    )
                    ...
                    for i in range(len(job_ids)):
                        ...
                        if polyester_units[i] is not None:
                            if (
                                polyester_units[i] == "polyester_printed_bag"
                                and float(no_of_pouch_kgs[i]) > 0
                            ):

                                total_ppb = float(purchase_rates[i]) / float(
                                    no_of_pouch_kgs[i]
                                )
                            else:
                        ...
                        job = get_object_or_404(
                            PouchQuotationJob, id=job_ids[i], quotation=edit_quotation
                        )
                        ...
                        job.zipper_cost = float(zipper_costs[i] or 0)
                        job.pouch_charge = float(pouch_charges[i] or 0)
                    ...
                    if "send_quotation_mail" in request.POST:
                        ...
                        return render(
                            request, "Purchase Order/purchase_order.html", context
                        )
...
def quotation_page_ajax(request):
    try:
        if request.method == "GET":
            ...
            jobs = list(
                PouchQuotationJob.objects.filter(
                    quotation__party_details__party_name=party_name
                )
                .values("job_name")
                .union(
                    PouchMaster.objects.filter(party_details__party_name=party_name)
                    .values("job_name")
                    .distinct()
                )
            )
            ...
            party_emails = list(
                PouchPartyEmail.objects.filter(party__party_name=party_name).values(
                    "email"
                )
            )
            party_contacts = list(
                PouchPartyContact.objects.filter(party__party_name=party_name).values(
                    "party_number"
                )
            )
            total_ppb = 0

            if purchase_rate_per_kg is not None:
                if unit == "polyester_printed_bag" and no_of_pouch_kg > 0:
        ...
        logger.exception("Error in quotation_page_ajax")
        print(e)
    return HttpResponse("")
